...
const PageBarber = require("../models/PageBarber").PageBarber;
...
router.get('/list', FUNC.Auth, async function (req, res) {
    ...
    let totalRecords = await PageBarber.countDocuments(searchQry);

    let pageData = await PageBarber.find(searchQry)
    ...
    async.eachSeries(pageData, function (page, loop) {
        ...
        temp.push({
            ...
            3: `<a href="/pageBarber/edit/${page._id}"title="Edit"><i class="fa fa-edit"></i></a>`
        });
        loop();
...
router.get('/edit/:id', FUNC.Auth, async function (req, res) {
    ...
    let page = await PageBarber.findOne({
        _id: params.id,
    });
    ...
    return res.render('page/editBarber', {
        title: "Edit Page",
        page,
    });
});
